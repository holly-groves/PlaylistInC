...
int insertBeforeCurr(Playlist* listPtr, char trackName[], int trackLength) {
	...
	if (listPtr->head == NULL && listPtr->tail == NULL) //if the playlist is empty
	{
	...
	{
		if (listPtr->curr->prev == NULL) //if the current track is at the head of the list
		{
...
int insertAfterCurr(Playlist* listPtr, char trackName[], int trackLength){
	...
	if (listPtr->head == NULL && listPtr->tail == NULL && listPtr->curr == NULL) //if the playlist is empty
	{
		listPtr->head == newTrack;
		listPtr->tail == newTrack;
		listPtr->curr == newTrack;
	...
	{
		if (listPtr->curr == listPtr->tail && listPtr->curr->next == NULL) //if the current track is at the tail of the list
		{
			newTrack->prev = listPtr->curr;
			listPtr->curr->next = newTrack;	
			listPtr->tail = newTrack;
		}
		else //if the current track is not at the tail of the list
...
int skipPrev(Playlist* listPtr){
	...
	if (listPtr == NULL) //if list is not initialised
		return INVALID_INPUT_PARAMETER;
	if (listPtr->curr == NULL  || listPtr->curr->next == NULL) //if list is empty or current track is at the head
		return INVALID_LIST_OPERATION;
...
int getCurrentTrack(Playlist* listPtr, MP3Track *pTrack){
	...
	if (pTrack == NULL) //check it is a valid pointer
		return INVALID_INPUT_PARAMETER;

	// if (listPtr->curr == NULL) //check that there is a track in the list to get info from
	// 	return INVALID_LIST_OPERATION;

	//store the track at current
	pTrack = listPtr->curr;
...
/*
	...
	LOOK AT HEADER FILE FOR MORE INFO
	parameters:
		listPtr -
		pTrack -
		moveForward -
